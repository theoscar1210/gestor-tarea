...
const USERNAME_KEY = "fintask_username";
...
const LoginPage = () => {
  const { login, registro, loginConToken } = useAuth();
  ...
  const handleSubmit = async (e) => {
    ...
    if (!resultado.success) {
      ...
      localStorage.setItem(USERNAME_KEY, resultado.username);
  ...
  const handleHuella = async () => {
    ...
    try {
      ...
      const resultado = await loginConToken(token, savedUsername);
      if (!resultado.success) {
        setError(resultado.mensaje);
      }
    } catch (err) {
            ...
            <form onSubmit={handleRegistro}>
              ...
              >
                {cargando ? (
                  ...
                  <><i className="bi bi-person-plus me-1" />Registrarse</>
                )}
              </button>
            </form>
